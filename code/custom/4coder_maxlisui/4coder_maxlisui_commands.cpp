...
global Face_ID maxlisui_small_italic_face;
global Face_ID maxlisui_minimal_face;


global b32 maxlisui_show_hex_colors;
global b32 maxlisui_relative_numbers;
global b32 maxlisui_show_scrollbars;

CUSTOM_COMMAND_SIG(maxlisui_toggle_show_hex_colors)
CUSTOM_DOC("Toggles value for `show_hex_colors`")
{ maxlisui_show_hex_colors ^= 1; }

CUSTOM_COMMAND_SIG(maxlisui_toggle_relative_numbers)
CUSTOM_DOC("Toggles value for `relative_numbers`")
{ maxlisui_relative_numbers ^= 1; }

CUSTOM_COMMAND_SIG(maxlisui_toggle_show_scrollbars)
CUSTOM_DOC("Toggles value for `show_scrollbars`")
{ maxlisui_show_scrollbars ^= 1; }

CUSTOM_COMMAND_SIG(maxlisui_test)
CUSTOM_DOC("Just bound to the key I spam to execute whatever test code I'm working on")
{
  ...
  maxlisui_reformat_buffer(app, buffer);
}

CUSTOM_COMMAND_SIG(maxlisui_clear_jumps)
CUSTOM_DOC("Clears jump buffer")
{
  unlock_jump_buffer();
}

CUSTOM_COMMAND_SIG(maxlisui_close_all_buffers)
...
CUSTOM_COMMAND_SIG(maxlisui_reload_config)
...
CUSTOM_COMMAND_SIG(maxlisui_reload_project)
...
CUSTOM_COMMAND_SIG(maxlisui_reopen_all_buffers)
...
CUSTOM_COMMAND_SIG(maxlisui_reset_face_size)
...
global Buffer_Cursor maxlisui_col_cursor;

CUSTOM_COMMAND_SIG(maxlisui_toggle_set_col_ruler)
CUSTOM_DOC("Toggles the column ruler. Set to cursor column when on.")
{
  View_ID view = get_active_view(app, Access_ReadVisible);
  if(maxlisui_col_cursor.pos == 0){
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
    i64 pos = view_get_cursor_pos(app, view);
    maxlisui_col_cursor = view_compute_cursor(app, view, seek_pos(pos));
    vim_state.params.seek = {buffer_get_char(app, buffer, pos), VIM_Inclusive, -1};
  }else{
    maxlisui_col_cursor = Buffer_Cursor{};
  }
}

CUSTOM_COMMAND_SIG(maxlisui_space)
CUSTOM_DOC("When column ruler is set, spaces towards that, else just inserts one space")
{
  ...
  if(maxlisui_col_cursor.pos > 0){
    ...
    maxlisui_col_cursor = buffer_compute_cursor(app, buffer, seek_line_col(maxlisui_col_cursor.line, maxlisui_col_cursor.col));

    i64 pos = view_get_cursor_pos(app, view);
    i64 line = get_line_number_from_pos(app, buffer, pos);
    f32 col_x = view_relative_xy_of_pos(app, view, line, maxlisui_col_cursor.pos).x;
...
global b32 maxlisui_bracket_opened;

CUSTOM_COMMAND_SIG(maxlisui_write_text_input)
CUSTOM_DOC("Inserts whatever text was used to trigger this command.")
{
  User_Input in = get_current_input(app);
  String_Const_u8 insert = to_writable(&in);
  maxlisui_bracket_opened = insert.str[insert.size-1] == '{';
  write_text(app, insert);
}

CUSTOM_COMMAND_SIG(maxlisui_auto_complete_bracket)
CUSTOM_DOC("Sets the right size of the view near the x position of the cursor.")
{
  ...
  do{
    if(token_array.tokens == 0){
      if(maxlisui_bracket_opened){
        write_text(app, string_u8_litexpr("\n\n}"));
        move_up(app);
        maxlisui_bracket_opened = 0;
        return;
      }else{
        break;
      }
    ...
    if(token && maxlisui_bracket_opened && buffer_get_char(app, buffer, token->pos) == '{'){
      token_it_dec(&it);
      ...
      insert.size -= (token->kind != maxlisui_TokenKind_Struct);
      write_text(app, insert);
      move_up(app);
      maxlisui_bracket_opened = 0;
      return;
...
function bool
maxlisui_is_divider(String_Const_u8 lexeme){
  return (string_starts_with(lexeme, string_u8_litexpr("//-"))  ||
          string_starts_with(lexeme, string_u8_litexpr("// -")) ||
          string_starts_with(lexeme, string_u8_litexpr("//--")));
}

function void
maxlisui_find_divider(Application_Links *app, Scan_Direction direction){
  View_ID view = get_active_view(app, Access_ReadVisible);
  Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
  ...
  for (;;){
    ...
    if (valid_dir && has_lexeme && maxlisui_is_divider(lexeme)){
...
CUSTOM_COMMAND_SIG(maxlisui_find_divider_up)
CUSTOM_DOC("Find //- divider above cursor")
{
  maxlisui_find_divider(app, Scan_Backward);
}

CUSTOM_COMMAND_SIG(maxlisui_find_divider_down)
CUSTOM_DOC("Find //- divider below cursor")
{
  maxlisui_find_divider(app, Scan_Forward);
}
...
function void
maxlisui_load_theme(String_Const_u8 theme_name){
  Color_Table *table_ptr = get_color_table_by_name(theme_name);
  if(table_ptr){ maxlisui_copy_color_table(&target_color_table, *table_ptr); }
}

global Vim_Buffer_Peek_Entry MAXLISUI_peek_list[VIM_ADDITIONAL_PEEK] = {
  { buffer_identifier(string_u8_litexpr("*scratch*")), 1.f, 1.f },
  { buffer_identifier(string_u8_litexpr("todo.txt")),  1.f, 1.f },
};

CUSTOM_COMMAND_SIG(maxlisui_list_all_locations_selection)
...
CUSTOM_COMMAND_SIG(maxlisui_open_current_peek)
...
VIM_TEXT_OBJECT_SIG(maxlisui_object_param){
...
VIM_TEXT_OBJECT_SIG(maxlisui_object_camel){
  Range_i64 range = {};
  Scratch_Block scratch(app);
...
function void maxlisui_make_vim_request(Application_Links *app, MAXLISUI_Vim_Request request){
  vim_make_request(app, Vim_Request_Type(VIM_REQUEST_COUNT + request));
}

VIM_REQUEST_SIG(maxlisui_apply_title){
  Scratch_Block scratch(app);
...
VIM_REQUEST_SIG(maxlisui_apply_comment){
...
VIM_REQUEST_SIG(maxlisui_apply_uncomment){
  i64 line0 = get_line_number_from_pos(app, buffer, range.min);
  i64 line1 = get_line_number_from_pos(app, buffer, range.max);
  line1 += (line0 == line1);
  History_Group history_group = history_group_begin(app, buffer);
...
VIM_COMMAND_SIG(maxlisui_request_title){ maxlisui_make_vim_request(app, MAXLISUI_REQUEST_Title); }
VIM_COMMAND_SIG(maxlisui_request_comment){ maxlisui_make_vim_request(app, MAXLISUI_REQUEST_Comment); }
VIM_COMMAND_SIG(maxlisui_request_uncomment){ maxlisui_make_vim_request(app, MAXLISUI_REQUEST_UnComment); }
VIM_COMMAND_SIG(maxlisui_visual_comment){
  if(vim_state.mode == VIM_Visual){
    Vim_Edit_Type edit = vim_state.params.edit_type;
    maxlisui_request_comment(app);
    vim_state.mode = VIM_Visual;
    vim_state.params.edit_type = edit;
  }
}
VIM_COMMAND_SIG(maxlisui_visual_uncomment){
  if(vim_state.mode == VIM_Visual){
    Vim_Edit_Type edit = vim_state.params.edit_type;
    maxlisui_request_uncomment(app);
    vim_state.mode = VIM_Visual;
    vim_state.params.edit_type = edit;
  }
}