function void
maxlisui_build_view_spin(Application_Links *app){
  for (;;){
...
function View_ID
maxlisui_get_or_open_build_panel(Application_Links *app){
  if(build_footer_panel_view_id == 0){
    ...
    view_enqueue_command_function(app, view, maxlisui_build_view_spin);
  }
  return build_footer_panel_view_id;
}

function void
maxlisui_prj_exec_command(Application_Links *app, Arena *scratch, Variable_Handle cmd_var){
  ...
  if (cmd.size > 0){
    ...
    if(out.size > 0){
      ...
      if(footer_panel){
        view = maxlisui_get_or_open_build_panel(app);
...
function void
maxlisui_prj_exec_command_fkey_index(Application_Links *app, i32 fkey_index){
  // setup fkey string
  ...
  maxlisui_prj_exec_command(app, scratch, cmd);
}


CUSTOM_COMMAND_SIG(maxlisui_project_fkey_command)
CUSTOM_DOC("Runs project commands but uses proper footer build panel")
{
  ...
  if(input.event.kind == InputEventKind_KeyStroke){
    ...
    if(got_ind){
      maxlisui_prj_exec_command_fkey_index(app, ind);
    }
  }
}

CUSTOM_COMMAND_SIG(maxlisui_build_project)
CUSTOM_DOC("Compiles project")
{
  View_ID view = get_active_view(app, Access_Always);
  Buffer_ID buffer = view_get_buffer(app, view, Access_Always);
  View_ID build_view = maxlisui_get_or_open_build_panel(app);
