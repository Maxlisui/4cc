...
function b32
maxlisui_is_theme(String_Const_u8 name){
...
maxlisui_init_color_table(Application_Links *app){
...
function void
maxlisui_copy_color_table(Color_Table *dst, Color_Table table){
...
function void
maxlisui_tick_colors(Application_Links *app, Frame_Info frame_info){
  ...
  if(active_color_table.arrays != cached_color_table.arrays){
    if(active_color_table.arrays && active_color_table.arrays->vals && active_color_table.arrays->count > 0){
      maxlisui_copy_color_table(&target_color_table, active_color_table);
...
function void
maxlisui_theme_navigate(Application_Links *app, View_ID view, Lister *lister, i32 index_delta){
  ...
  if(result){ maxlisui_copy_color_table(&target_color_table, *result); }
}


CUSTOM_UI_COMMAND_SIG(maxlisui_theme_lister)
CUSTOM_DOC("Opens an interactive list of all registered themes.")
{
  ...
  if(color_table_list->first){
    maxlisui_copy_color_table(&target_color_table, color_table_list->first->table);
  }
  ...
  lister.lister.current->handlers.navigate = maxlisui_theme_navigate;
  ...
  if(result != 0){ maxlisui_copy_color_table(&target_color_table, *result); }
}