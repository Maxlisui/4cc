...
function b32
maxlisui_highlight_token(Token_Base_Kind kind){
  switch(kind){
    case TokenBaseKind_Keyword:
    //case TokenBaseKind_Preprocessor:
    case TokenBaseKind_Identifier:
    case maxlisui_TokenKind_Primitive:
    case maxlisui_TokenKind_ControlFlow:
    case maxlisui_TokenKind_Struct:
    return true;
  }
  return false;
}
...
function ARGB_Color
maxlisui_get_token_color_cpp(Token token){
  ...
  switch (token.kind){
    ...
    case maxlisui_TokenKind_ControlFlow:{ color = defcolor_control; }break;
    case maxlisui_TokenKind_Primitive:{ color = defcolor_primitive; }break;
    case maxlisui_TokenKind_Struct:{ color = defcolor_struct; }break;
  }
...
function void
maxlisui_draw_cpp_token_colors(Application_Links *app, Text_Layout_ID text_layout_id, Token_Array *array){
  ...
  for(;;){
    ...
    ARGB_Color argb = maxlisui_get_token_color_cpp(*token);
    paint_text_color(app, text_layout_id, Ii64_size(token->pos, token->size), argb);
    if(!token_it_inc_all(&it)){ break; }
  }
}

function void maxlisui_draw_token_colors(Application_Links *app, View_ID view, Buffer_ID buffer, Text_Layout_ID text_layout_id){
  Token_Array token_array = get_token_array_from_buffer(app, buffer);
  Range_i64 visible_range = text_layout_get_visible_range(app, text_layout_id);

  Scratch_Block scratch(app);
  maxlisui_draw_cpp_token_colors(app, text_layout_id, &token_array);
  ...
  b32 do_cursor_tok_highlight = maxlisui_highlight_token(cursor_token->kind);
  ...
  ARGB_Color cursor_tok_color = maxlisui_get_token_color_cpp(*cursor_token);
