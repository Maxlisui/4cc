CUSTOM_COMMAND_SIG(maxlisui_startup)
CUSTOM_DOC("Responding to a startup event")
{
  ProfileScope(app, "maxlisui startup");
  ...
  if(match_core_code(&input, CoreCode_Startup)){
    ...
    maxlisui_get_or_open_build_panel(app);
  ...
  maxlisui_small_italic_face = try_create_new_face(app, &desc);

  desc.parameters.pt_size = 0;
  desc.parameters.bold = 0;
  maxlisui_minimal_face = try_create_new_face(app, &desc);

  system_set_fullscreen(false);
  set_window_title(app, string_u8_litexpr("4coder Maxlisui"));

  maxlisui_relative_numbers = 1;
  maxlisui_show_hex_colors = 1;
  maxlisui_show_scrollbars = 0;

  Color_Table *table = get_color_table_by_name(string_u8_litexpr("theme-maxlisui"));
  if(table == 0){ table = &default_color_table; }
  target_color_table = maxlisui_init_color_table(app);
  cached_color_table = maxlisui_init_color_table(app);
  maxlisui_copy_color_table(&target_color_table, *table);
  maxlisui_copy_color_table(&cached_color_table, *table);
  active_color_table = cached_color_table;
}

function void
maxlisui_tick(Application_Links *app, Frame_Info frame_info){
  ProfileScope(app, "maxlisui tick");
  ...
  maxlisui_tick_colors(app, frame_info);
...
function void
maxlisui_render_caller(Application_Links *app, Frame_Info frame_info, View_ID view_id){
  ProfileScope(app, "maxlisui render caller");
  ...
  Rect_f32_Pair scrollbar_pair = maxlisui_layout_scrollbars(region, digit_advance);
  i64 show_scrollbar = false;
  view_get_setting(app, view_id, ViewSetting_ShowScrollbar, &show_scrollbar);
  show_scrollbar &= maxlisui_show_scrollbars;
  if(show_scrollbar){
  ...
  if(show_scrollbar){
    maxlisui_draw_scrollbars(app, view_id, buffer, text_layout_id, scrollbar_pair.b);
  }

  maxlisui_render_buffer(app, view_id, face_id, buffer, text_layout_id, region);
...
function Rect_f32
maxlisui_buffer_region(Application_Links *app, View_ID view_id, Rect_f32 region){
  ProfileScope(app, "maxlisui buffer region");
  Buffer_ID buffer = view_get_buffer(app, view_id, Access_Always);
  Face_ID face_id = get_face_id(app, 0);
  ...
  i64 show_scrollbar = false;
  view_get_setting(app, view_id, ViewSetting_ShowScrollbar, &show_scrollbar);
  if(maxlisui_show_scrollbars && show_scrollbar){
    region = maxlisui_layout_scrollbars(region, digit_advance).a;
  }
...
BUFFER_HOOK_SIG(maxlisui_file_save){
  ...
  if (auto_indent && is_virtual){
    ...
    maxlisui_reformat_buffer(app, buffer_id);
  }
  ...
  if (maxlisui_is_theme(name)){
    ...
    if (error_text.size > 0){
      ...
      maxlisui_copy_color_table(&target_color_table, color_table);
    }
  }

  if (string_match(name, string_u8_litexpr("config.4coder"))){
    View_ID view = get_active_view(app, Access_Always);
    view_enqueue_command_function(app, view, maxlisui_reload_config);
  }
...
BUFFER_HOOK_SIG(maxlisui_begin_buffer){
  ProfileScope(app, "maxlisui begin buffer");
...
BUFFER_HOOK_SIG(maxlisui_new_file){
  Scratch_Block scratch(app);
