function void
maxlisui_draw_function_preview_inner(Application_Links *app, Buffer_ID buffer, Range_f32 x_range, Range_i64 range, i64 pos, i32 count){
  ...
  Face_Metrics metrics = get_face_metrics(app, maxlisui_small_italic_face);
  f32 wid = 2.f;
  f32 pad = 2.f;
  Rect_f32 rect = {};
  ...
  draw_string(app, maxlisui_small_italic_face, sig, rect.p0 + (pad + wid)*V2f32(1,1), fcolor_id(defcolor_ghost_character));
}

function void
maxlisui_draw_function_preview(Application_Links *app, Buffer_ID buffer, Range_f32 x_range, i64 pos){
  ...
  foreach(i, ranges.count){
    ...
    if(token->kind == TokenBaseKind_Identifier){
      maxlisui_draw_function_preview_inner(app, buffer, x_range, ranges.ranges[i], pos, count++);
    }
  }
}

function void
maxlisui_hex_color_preview(Application_Links *app, Buffer_ID buffer_id, Text_Layout_ID text_layout_id){
  Scratch_Block scratch(app);
...
maxlisui_layout_scrollbars(Rect_f32 region, f32 digit_advance){ return rect_split_left_right_neg(region, 1.25f*digit_advance); }

function void
maxlisui_draw_scrollbars(Application_Links *app, View_ID view, Buffer_ID buffer, Text_Layout_ID text_layout_id, Rect_f32 region){
...
function void
maxlisui_draw_scope_brackets(Application_Links *app, View_ID view, Buffer_ID buffer, Text_Layout_ID text_layout_id, Rect_f32 region, i64 pos){
  ...
  for(i32 i=ranges.count-1; i >= 0; --i){
      ...
      if(paren_level == 0){
           ...
           (token->kind == TokenBaseKind_StatementClose && token->sub_kind != TokenCppKind_Colon))
        {
          break;
                ...
                token->kind == maxlisui_TokenKind_ControlFlow)
        {
          start_token = token;
          break;
        }
    ...
    if(start_token){
      ...
      draw_string(app, maxlisui_small_italic_face, start_line_text, string_pos, fcolor_id(defcolor_ghost_character));
    }
  }
}

function void
maxlisui_draw_comments(Application_Links *app, Buffer_ID buffer, Text_Layout_ID text_layout_id, Token_Array *array, Rect_f32 rect){
  Scratch_Block scratch(app);
...
function void
maxlisui_draw_compile_errors(Application_Links *app, Buffer_ID buffer, Text_Layout_ID text_layout_id, Buffer_ID comp_buffer){
  Scratch_Block scratch(app);
  Range_i64 visible_range = text_layout_get_visible_range(app, text_layout_id);
  ...
  for (i32 i = 0; i < count; i += 1){
    ...
    draw_string(app, maxlisui_small_italic_face, error_string, p0, cl_error);
  }
}

function void
maxlisui_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id, Buffer_ID buffer, Text_Layout_ID text_layout_id, Rect_f32 rect){
  ...
  if(is_active_view && maxlisui_col_cursor.pos > 0){
    ...
    Vec2_f32 col_rel = view_relative_xy_of_pos(app, view_id, line, maxlisui_col_cursor.pos);
  ...
  if(token_array.tokens == 0){
    ...
    maxlisui_draw_token_colors(app, view_id, buffer, text_layout_id);
    maxlisui_draw_comments(app, buffer, text_layout_id, &token_array, rect);
  }
  ...
  if(use_error_highlight || use_jump_highlight){
    Buffer_ID comp_buffer = get_buffer_by_name(app, string_u8_litexpr("*compilation*"), Access_Always);
    if(use_error_highlight && comp_buffer){
      maxlisui_draw_compile_errors(app, buffer, text_layout_id, comp_buffer);
    }
  ...
  maxlisui_draw_scope_brackets(app, view_id, buffer, text_layout_id, rect, cursor_pos);
  ...
  if(maxlisui_show_hex_colors){
    maxlisui_hex_color_preview(app, buffer, text_layout_id);
  }
  ...
  if(is_active_view){
    maxlisui_draw_function_preview(app, buffer, If32(rect.x0, rect.x1), cursor_pos);
  }
